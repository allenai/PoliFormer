...
class Attention(nn.Module):
    ...
    def sampler_select(self, keep: list):
        if self.cache_k.shape[0] == 1 and len(keep) > 1:
            self.cache_k = self.cache_k.repeat(len(keep), 1, 1, 1) * 0
            self.cache_v = self.cache_v.repeat(len(keep), 1, 1, 1) * 0
        else:
            self.cache_k = self.cache_k[keep]
            self.cache_v = self.cache_v[keep]
    ...
    def forward(
        ...
        """
        if self.cache_v.device != x.device:
        ...
        if seqlen == 1:
            if self.cache_k.shape[0] == 0 and bsz > 0:
                self.cache_k = torch.zeros((bsz, *self.cache_k.shape[1:]))
                self.cache_v = torch.zeros((bsz, *self.cache_v.shape[1:]))
